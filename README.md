...
- 横屏与竖屏镜头相对地面法线的倾角；推荐值分别是 40° 和 35°。
- 正交镜头缩放、环境光、主光以及减少动态效果。
- “恢复推荐值”可以一键撤销本轮临时调整。

Three.js 中 `Y` 轴是上下方向，`X` 与 `Z` 轴构成地面；面板中的镜头倾角就是视线偏离 `Y` 轴的角度。该面板只在开发服务器中创建，`npm run build` 的正式页面不会显示。
...
- “随机地图实验室”提供林间空地、蜿蜒果径、交错林路三种预置；相同种子、密度和道路宽度会生成完全相同的四个候选。
- 画布支持种树、擦除、铺路、画空地、放置果实和移动三个角色起点与投递区；小路和空地会自动清理附近树木。
- 编辑过程支持撤销与重做。可游玩检查会验证树木/果实数量、边界和全部关键目标的可达性，不合法地图不能进入游戏。

...
// Reuse the user's installed stable Chrome. The project deliberately avoids a
// second Playwright-managed Chromium download for local visual verification.
async function launchBrowser() {
  try {
    return await chromium.launch({ channel: 'chrome' });
  } catch (error) {
    throw new Error(
      `Installed Google Chrome could not be launched through Playwright channel:"chrome". ${error}`,
    );
  }
}

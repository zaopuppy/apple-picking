...
项目提供面向桌面浏览器的双人联机演示：一名玩家选择 `guards` 席位并同时控制两名守卫，另一名玩家选择 `kid` 席位控制 kid。游戏规则、碰撞、计分和胜负由 Node.js + Socket.IO 权威服务端统一推进。

### 创建和加入房间

1. 在主机运行 `npm run dev`。
2. 第一位玩家打开 <http://127.0.0.1:5188/online.html>，选择席位并创建房间。
3. 把页面显示的六位房间码发给另一位玩家。
4. 第二位玩家打开同一联机入口，选择另一个席位并输入房间码。
5. 双方点击“准备”后进入倒计时并开始比赛。

同一台电脑可使用两个浏览器标签页测试。局域网试玩时，其他设备访问 `http://<主机局域网 IP>:5188/online.html`；Vite 和多人服务默认分别监听 `0.0.0.0:5188` 与 `0.0.0.0:5190`。

房间服务支持以下环境变量：

| 变量 | 默认值 | 用途 |
| --- | --- | --- |
| `APPLE_PICKING_SERVER_HOST` | `0.0.0.0` | HTTP 与 Socket.IO 监听地址 |
| `APPLE_PICKING_SERVER_PORT` | `5190` | HTTP 与 Socket.IO 监听端口 |

修改多人服务端口时，还要同步修改 [vite.config.ts](vite.config.ts) 中 `/socket.io` 的开发与预览代理目标。

### 同步模型

- 服务端以 60 tick/s 运行每个房间的权威 `GameSimulation`，客户端只发送自己席位的输入意图。
- 客户端以约 30 Hz 发送输入，服务端以 20 Hz 广播全量状态。
- 远端角色使用约 100 ms 的快照缓冲插值。
- 本地角色从带版本的 `SimulationCheckpoint` 恢复完整规则状态，并重放尚未确认的输入；视觉纠偏不会改变权威结果。
- 意外断线会暂停房间，并在 15 秒内尝试使用私有 rejoin token 恢复原席位。
- 在线地图由服务端发布，浏览器本地地图不会覆盖房间地图。

### 演示边界

当前联机版使用 HTTP + WebSocket，不配置 SSL/TLS，适合本机双标签或受信任局域网。房间保存在单个 Node.js 进程的内存中，进程重启会结束房间；当前不包含账号、公开匹配、持久化、观战或多实例扩容，不应原样暴露到公网。

详细设计、协议与阶段验证记录见 [桌面网页联机架构文档](docs/2026-08-16_desktop-web-multiplayer-architecture-discussion.md)。

## 参与开发

开发环境、目录职责、确定性与联机约束、测试矩阵以及提交和 Pull Request 规范见 [贡献指南](CONTRIBUTING.md)。
...
在线模式复用同一套键位，但每个浏览器只提交自己席位拥有的输入，房间使用方法见前文“多人联机”。

...
//
// Requires window.__THREE_GAME_DIAGNOSTICS__ (frame/delivered/matchState/kid) and
...
// Keyboard sweep placeholder for the kid path. Add explicit Right Control
// pickup taps after tuning this path against the authored apple positions.
// Each step holds keys for `ms` then samples diagnostics.
const INPUT_SCRIPT: Array<{ keys: string[]; ms: number }> = [
  { keys: ['ArrowUp'], ms: 1000 },
  { keys: ['ArrowLeft'], ms: 1900 },
  { keys: ['ArrowRight'], ms: 3400 },
  { keys: ['ArrowUp'], ms: 350 },
  { keys: ['ArrowLeft'], ms: 3400 },
  { keys: ['ArrowDown'], ms: 1700 },
  { keys: ['ArrowRight'], ms: 3400 },
  { keys: ['ArrowDown'], ms: 350 },
  { keys: ['ArrowLeft'], ms: 3400 },
];
...
test('bot playtest: scripted input drives progress without errors', async ({ page }, testInfo) => {
  ...
  const sample = (): Promise<BotSnapshot | null> =>
    page.evaluate(() => {
      ...
      return {
        ...
        complete: d.matchState === 'KidWin' || d.matchState === 'GuardWin',
        x: d.kid.position.x,
        z: d.kid.position.z,
      };
    });
  ...
  for (const [index, step] of INPUT_SCRIPT.entries()) {
    ...
    if (snap) {
      ...
      distance += moved;
      const progressed = snap.delivered > prev.delivered || (snap.complete && !prev.complete);
  ...
  const report = {
    ...
    scoreBefore: (before as BotSnapshot).delivered,
    scoreAfter: after.delivered,

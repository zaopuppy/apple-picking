...
在线版使用 100 ms 远端快照插值和本地拥有角色的表现层预测，移动输入不再等待 20 Hz 权威快照才显示；碰撞、拾取、计分和胜负仍完全由服务端确认。它仍是 HTTP + WebSocket 的纯演示，不包含 SSL/TLS、账号、公开匹配、持久化或完整客户端模拟 checkpoint，适合本机双标签或受信任局域网试玩，不应原样暴露到公网。
